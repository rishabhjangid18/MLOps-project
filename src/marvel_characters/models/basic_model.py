...
class BasicModel:
    ...
    def model_improved(self) -> bool:
        """
        Compare current model with the latest registered model using F1-score.
        Handles first-run and alias-missing cases safely.
        """
        client = MlflowClient()

        # -----------------------------
        # 1️⃣ Check if registered model exists
        # -----------------------------
        try:
            client.get_registered_model(self.model_name)
            logger.info(f"Registered model exists: {self.model_name}")
        except RestException:
            logger.warning("No registered model found. First run detected.")
            return True  # First model is always considered improved

        # -----------------------------
        # 2️⃣ Check if alias exists
        # -----------------------------
        try:
            ...
            logger.info(
                f"Alias 'latest-model' found → version={latest_model_version.version}"
            )
        except RestException:
            logger.warning(
                "Alias 'latest-model' does not exist. Treating current model as improved."
            )
            return True
        ...
        old_f1 = result.metrics.get("f1_score", 0)
        new_f1 = self.metrics.get("f1_score", 0)

        logger.info(f"Model comparison → old_f1={old_f1:.4f}, new_f1={new_f1:.4f}")

        if new_f1 >= old_f1:
            logger.info("Current model improves over latest → returning True")
            return True
        else:
            logger.info("Current model does not improve → returning False")
            return False
